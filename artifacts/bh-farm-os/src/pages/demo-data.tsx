...
export default function DemoDataPage() {
  ...
  const load = async () => {
    const response = await fetch(api("/api/demo/status"));
  ...
  const seed = async () => {
    ...
    try {
      const response = await fetch(api("/api/demo/seed"), { method: "POST" });
  ...
  const reset = async () => {
    if (!window.confirm("Remove only BH Farm OS demo data? Real farm records are not targeted.")) return;
    setBusy(true); setMessage("");
    try {
      const response = await fetch(api("/api/demo"), { method: "DELETE" });
      const body = await response.json();
  ...
  return <div className="animate-rise space-y-5">
    ...
    <Panel className="p-5"><div className="flex items-start gap-3"><ShieldCheck size={19} className="mt-0.5 text-[hsl(var(--primary))]" /><div><h2 className="font-semibold">Production safety rule</h2><p className="mt-1 text-sm text-[hsl(var(--muted-foreground))]">This tool never clears the farm database. The reset operation targets only records created by this demo lifecycle.</p></div></div></Panel>
  </div>;
}
