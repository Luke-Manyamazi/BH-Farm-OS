import { ArrowRight, BarChart3, Bird, CalendarDays, CircleAlert, CircleDollarSign, ClipboardList, Droplets, Egg, Fish, FlaskConical, Gauge, Grid3X3, HeartPulse, Home, Leaf, Map, Package, PiggyBank, Sprout, Tractor, TreePine, Users, Wheat, Wrench } from "lucide-react";
import { useLocation } from "wouter";
import { PageHeading } from "@/components/farm-shell";
import { Panel } from "@/components/ui-kit";

type Card = { title: string; description: string; href: string; Icon: typeof Home; tone?: string };

type CardGroup = { title: string; eyebrow: string; cards: Card[] };

const groups: CardGroup[] = [
  {
    eyebrow: "Run the farm",
    title: "Field book",
    cards: [
...
function HomeCard({ card }: { card: Card }) {
  ...
  return (
    <button
      ...
      className="group flex min-h-[170px] flex-col rounded-2xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] p-5 text-left shadow-[var(--shadow-sm)] transition duration-200 hover:-translate-y-0.5 hover:border-[hsl(var(--primary)/.4)] hover:shadow-[var(--shadow-md)] focus:outline-none focus:ring-2 focus:ring-[hsl(var(--primary)/.35)]"
      data-testid={`home-card-${card.title.toLowerCase().replace(/[^a-z0-9]+/g, "-")}`}
    >
      <div className="flex items-start justify-between gap-3">
        <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-[hsl(var(--secondary))] text-[hsl(var(--primary))] transition group-hover:bg-[hsl(var(--primary))] group-hover:text-[hsl(var(--primary-foreground))]">
          <Icon size={20} />
        </div>
        <ArrowRight size={17} className="mt-1 text-[hsl(var(--muted-foreground))] transition group-hover:translate-x-1 group-hover:text-[hsl(var(--primary))]" />
      </div>
      <div className="mt-auto pt-7">
        <h3 className="font-[family-name:var(--app-font-serif)] text-lg font-bold tracking-tight">{card.title}</h3>
        <p className="mt-1.5 text-[11px] leading-relaxed text-[hsl(var(--muted-foreground))]">{card.description}</p>
      </div>
    </button>
  );
}
...
export function FarmHomePage() {
  ...
  return (
    <div className="animate-rise space-y-8">
      <PageHeading
        eyebrow="Boroma Hills · ZW"
        title="BH Farm OS"
        detail="Your field book for livestock, crops, water, resources, money, and the work that keeps the farm moving."
        action={<button type="button" onClick={() => setLocation("/search")} className="inline-flex items-center gap-2 rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] px-3 py-2 text-[11px] font-semibold hover:bg-[hsl(var(--secondary))]">Search farm</button>}
      />

      <Panel className="overflow-hidden">
        <div className="grid gap-5 bg-[hsl(var(--primary))] p-5 text-[hsl(var(--primary-foreground))] sm:p-7 lg:grid-cols-[1fr_auto] lg:items-center">
          <div>
            <div className="font-mono text-[9px] uppercase tracking-[.18em] text-white/60">Field book</div>
            <h2 className="mt-2 font-[family-name:var(--app-font-serif)] text-3xl font-bold tracking-tight sm:text-4xl">One home for the whole farm.</h2>
            <p className="mt-2 max-w-2xl text-sm leading-relaxed text-white/70">Use these cards as the starting point for every part of Boroma Hills. Each card opens the working page where records are created, updated, and reviewed.</p>
          </div>
          <div className="rounded-2xl border border-white/15 bg-white/[.08] px-5 py-4 lg:min-w-[170px]">
            <div className="font-mono text-[9px] uppercase tracking-[.16em] text-white/55">Operating modules</div>
            <div className="mt-1 font-[family-name:var(--app-font-serif)] text-3xl font-bold">{totalCards}</div>
            <div className="text-[10px] text-white/60">working entry points</div>
          </div>
        </div>
      </Panel>
      ...
      {groups.map(group => (
        <section key={group.title}>
          <div className="mb-3 flex items-end justify-between gap-4">
            <div>
              <div className="font-mono text-[9px] uppercase tracking-[.17em] text-[hsl(var(--primary))]">{group.eyebrow}</div>
              <h2 className="mt-1 font-[family-name:var(--app-font-serif)] text-2xl font-bold tracking-tight">{group.title}</h2>
            </div>
            <div className="hidden text-[10px] text-[hsl(var(--muted-foreground))] sm:block">{group.cards.length} modules</div>
          </div>
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
            {group.cards.map(card => <HomeCard key={card.href} card={card} />)}
          </div>
        </section>
      ))}
    </div>
